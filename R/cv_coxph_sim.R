...
#' @export
simul_coxph_causal_cv <- function(N = 500, # n = 250,
                               X,
    ...
    final_pred_0_RCT <- matrix(NA, nrow = sum(RCT == 1), ncol = length(fit.times))
    final_pred_0_Obs <- matrix(NA, nrow = sum(RCT == 0), ncol = length(fit.times))
    final_pred_1_RCT <- matrix(NA, nrow = sum(RCT == 1), ncol = length(fit.times))
    final_pred_1_Obs <- matrix(NA, nrow = sum(RCT == 0), ncol = length(fit.times))
    ...
    if(run_type == "correct"){
        ...
        for(v in 1:V) {
            ...
            S.hats.1_temp <- t(summary(predicted_1, times = fit.times)$surv)
        ...
        pred_0 <- rbind(final_pred_0_RCT, final_pred_0_Obs)
        pred_1 <- rbind(final_pred_1_RCT, final_pred_1_Obs)

        final_pred_0 <- as_tibble(cbind(fit.times, apply(pred_0,2, mean),out_S0))
        colnames(final_pred_0) <- c('time','surv','true.surv')

        final_pred_1 <- as_tibble(cbind(fit.times, apply(pred_1,2, mean),out_S1))
    ...
    if(run_type == "incorrect"){
        ...
        for(v in 1:V) {
            ...
            new_data <- as.data.frame( rbind(
                cbind(A[RCT == 1][test], X_mod[RCT == 1,][test,]),
...
#
# ex_simul_cv <- simul_coxph_causal_cv(N = N, # n = n,
#                                      X = X,
#                                      beta_R = beta_R,
#                                      beta_A = beta_A,
#                                      beta_T = beta_T,
...
#                                      fit.times = seq(0,10, by = 1)
# )
# ex_simul_cv$Delta_0_bias[5]
# ex_simul_cv$Delta_1_bias[5]
#
#
...
#
# ex_simul_cf <- simul_DML_causal(N = N, # n = n,
#                                 X = X,
#                                 beta_R = beta_R,
#                                 beta_A = beta_A,
#                                 beta_T = beta_T,
...
#                                 fit.times = seq(0,10, by = 1)
# )
# ex_simul_cf$Delta_0_bias[5]
# ex_simul_cf$Delta_1_bias[5]
