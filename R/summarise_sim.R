...
summarise_sim <- function(CF_sim_obs, t = NULL){
    ...
    # delta #
    ## Bias ##
    bias0 <- mean(sapply(CF_sim_obs, "[[", "Delta_0_bias")[t*10,], na.rm = T)
    bias1 <- mean(sapply(CF_sim_obs, "[[", "Delta_1_bias")[t*10,], na.rm = T)
    ## MSE ##
    MSE0 <- mean(sapply(CF_sim_obs, "[[", "Delta_0_MSE")[t*10,], na.rm = T)
    MSE1 <- mean(sapply(CF_sim_obs, "[[", "Delta_1_MSE")[t*10,], na.rm = T)

    # Quantiles #
    ## Bias ##
    quant_bias0 <- apply((sapply(CF_sim_obs, "[[","quant_bias_0")),1, function(x){mean(x, rm.na = T)})
    quant_bias1 <- apply((sapply(CF_sim_obs, "[[","quant_bias_1")),1, function(x){mean(x, rm.na = T)})
    ## MSE ##
    quant_MSE0 <- apply((sapply(CF_sim_obs, "[[","quant_MSE_0")),1, function(x){mean(x, rm.na = T)})
    quant_MSE1 <- apply((sapply(CF_sim_obs, "[[","quant_MSE_1")),1, function(x){mean(x, rm.na = T)})
